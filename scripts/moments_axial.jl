...
@time for ii in range(1, Ndata)
    ...
    for jj in range(1, length(powers))       
        moments_nz0[ii, jj] = ψ_nz0' * (get_periodic_zpower(zz, powers[jj]) .* ψ_nz0)
        moments_nz1[ii, jj] = ψ_nz1' * (get_periodic_zpower(zz, powers[jj]) .* ψ_nz1)
    end
...
fig_nz1 = plot(depths[depths .> 8], abs.(moments_nz1[depths .> 8, :]), xticks= ([ 7, 15, 30, 100], string.([7, 15, 30, 100])), st=:scatter, 
xscale=:log10, 
xlims=(2, 200),
ylims=(-0.1, 1.1),
# yscale=:log10,
