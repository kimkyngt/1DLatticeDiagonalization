...
function get_reduction_factor_sag(V0)
    ...
    ωR = uconvert(u"m*s^-1",√(4*V0*Er/m87Sr))/w_0
    A  = uconvert(u"m^-2", 1/2*m87Sr*ωR^2/k_B/Tr)
    Ap = 2/w_0^2 
    B = uconvert(u"m^-1", m87Sr*9.796u"m/s^2"*sin(θtilt)/(k_B*Tr))
...
function get_center_reduction(V0)
    """V0 in units of Er = no units"""
    ζ = exp( -uconvert(NoUnits, (m87Sr*9.796u"m/s^2"*sin(θtilt)*w_0)^2/8/Er^2) /(V0- √(V0))^2)
   return ζ
end
