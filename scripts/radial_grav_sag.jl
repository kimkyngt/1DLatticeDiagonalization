...
plot!(rr * 1e6, -U_0 * exp.(-2 * rr .^ 2 / w_0^2) .+ ustrip(m87Sr * g_0 * sin(θtilt) / Er) * rr, label="-U(x)/Eᵣ + mgsin(θ)x", minorticks=5, size=(600, 300), lc=:black, lw=1)
...
plot!(xlabel=L"\rho" * " (μm)", ylabel=L"U_{n_z}(\rho)", label=L"\Delta \rho", ls=:solid, fill=0, alpha=0.5, lw=1, legend=:best)
