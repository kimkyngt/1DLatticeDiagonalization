...
zz = range(-1.5*3, 1.5*3, length=300)
A = 0.2
B = 0.1
U = -0.3*(cos.(π*zz).^2)
mgz = B*zz
...
annotate!(-1.5, -0.1+offset+0.03, ("³P₀", 8, :right, :black))
annotate!(-1.5, -0.1+0.03, ("¹S₀", 8, :right, :black))
...
# fine tune
plot!(axis=false, legend=false, grid=false, ticks=false, xlims=(-2.5, 2), size = ((3+3/8)*96, (3+3/8)*96*2.3/4),
)

# Plots.pdf(plotsdir("lattice_diagram_without_tilt.pdf"))
plot!()