...
function integrand(U_0, T_r, r, t)
    get_rho(r, U_0, T_r, 1)*sin(rabi_1(U_0*exp(-2*r^2/w_0^2))*t/2)^2
end
...
function get_normalziation(U_0, T_r)
    N, err = quadgk(r -> r*get_rho(r, U_0, T_r, 1), 0, rmax)
    return N
end

# Parameters
U_0 = 18
T_r = get_Tr(U_0)*0.6#80u"nK"
w_0 = 260e-6 # cavity waist
rmax = w_0 # in 

# fig1 density
rr = range(0, rmax, length=100)
fig_density = plot(rr*1e6, [get_rho(r, U_0, T_r, 1) for r in rr] / get_rho(0, U_0, T_r, 1), xlabel="r (μm)", label="ρ")
plot!(rr*1e6, rabi_1.(U_0*exp.(-2*rr.^2/w_0^2)), color=2, ylims=(0, 1.1) , label="Ω/Ω₀")
...
# fig2 rabi flopping
tt = collect(range(0, 9π, length=100))
...
annotate!(2, 1.05, L"$Ω/Ω_{fit} = $"*string(round(rabi_1(U_0)/fit.param[4], digits=2)), 9)
